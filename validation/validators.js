...
function validateColumns(equipmentList, fieldMap, errors) {
  ...
  for (col = 0; col < headers.length; col++) {
    ...
    if (field) {
      ...
      //if filed is undefined it should be an unknown field.
      errors.push(new LineError('Unknown Field Detected', headers[col], 'N/A'));
    }
  }
...
function validateData(equipmentList, fieldMap, errors) {
  //counter variable should not be a block variable otherwise it'll not be incremented
  let counter = 2;
